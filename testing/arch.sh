...
dialog --defaultno --title "Time Zone select" --yesno "Do you want use the default time zone(Europe/Berlin)?.\n\nPress no for select your own time zone"  10 60 && echo "Europe/Berlin" > tz.tmp || tzselect > tz.tmp
...
curl https://raw.githubusercontent.com/callamaris/LARBS/master/testing/chroot.sh > /mnt/chroot.sh && arch-chroot /mnt bash chroot.sh && rm /mnt/chroot.sh
